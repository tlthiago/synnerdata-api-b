...
|-----------|---------|-----------------|
| `loggerPlugin` | `src/plugins/request-logger/logger-plugin.ts` | Access log (todas as requests), `X-Request-ID` header, `requestId` no contexto |
| `logger` (Pino util) | `src/lib/logger.ts` | Instância Pino + `shouldIgnore` + `generateRequestId` (utilitário puro, sem lifecycle Elysia) |
| `errorPlugin` | `src/plugins/error-handler/error-plugin.ts` | Error log (detalhes de 5xx e unhandled), formatação do envelope de erro |
