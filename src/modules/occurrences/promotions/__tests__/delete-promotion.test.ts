...
describe("DELETE /v1/promotions/:id", () => {
  let app: TestApp;
  ...
  test("should reject when promotion is already deleted", async () => {
    ...
    expect(response.status).toBe(404);
    const body = await response.json();
    expect(body.error.code).toBe("PROMOTION_ALREADY_DELETED");
  });
  ...
  test("should allow supervisor to delete promotion", async () => {
    const { organizationId, user } = await createTestUserWithOrganization({
      emailVerified: true,
    });
  ...
  test("should reject viewer from deleting promotion", async () => {
    const { organizationId, user } = await createTestUserWithOrganization({
      emailVerified: true,
    });
