...
describe("PUT /v1/ppe-items/:id", () => {
  ...
  test("should update ppe item name", async () => {
  ...
  test("should update ppe item description and equipment", async () => {
    ...
    expect(body.data.equipment).toBe("Novo equipamento");
  });

  test("should not update ppe item from another organization", async () => {
  ...
  test.each([
    "viewer",
    "supervisor",
  ] as const)("should reject %s from updating ppe item", async (role) => {
  ...
  test("should allow manager to update ppe item", async () => {
    const { addMemberToOrganization } = await import(
      "@/test/helpers/organization"
    );
