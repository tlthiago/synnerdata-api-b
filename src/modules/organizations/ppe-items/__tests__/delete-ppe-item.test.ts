...
describe("DELETE /v1/ppe-items/:id", () => {
  let app: TestApp;
  ...
  test("should soft delete ppe item", async () => {
  ...
  test("should return 404 when trying to delete already deleted ppe item", async () => {
    ...
    expect(response.status).toBe(404);
    const body = await response.json();
    expect(body.error.code).toBe("PPE_ITEM_ALREADY_DELETED");
  });

  test("should not delete ppe item from another organization", async () => {
  ...
  test.each([
    "viewer",
    "supervisor",
  ] as const)("should reject %s from deleting ppe item", async (role) => {
  ...
  test("should allow manager to delete ppe item", async () => {
    const { addMemberToOrganization } = await import(
      "@/test/helpers/organization"
    );
