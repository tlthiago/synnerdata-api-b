...
const validCostCenterData = {
  name: "Centro de Custo Administrativo",
};

describe("POST /v1/cost-centers", () => {
  let app: TestApp;
  ...
  test("should reject unauthenticated requests", async () => {
    const response = await app.handle(
      new Request(`${BASE_URL}/v1/cost-centers`, {
        ...
        body: JSON.stringify(validCostCenterData),
      })
    );
  ...
  test.each([
    "viewer",
    "supervisor",
  ] as const)("should reject %s member from creating cost center", async (role) => {
  ...
  test("should allow manager to create cost center", async () => {
    const { addMemberToOrganization } = await import(
      "@/test/helpers/organization"
    );
    ...
    const response = await app.handle(
      new Request(`${BASE_URL}/v1/cost-centers`, {
        ...
        body: JSON.stringify({ name: "Centro de Custo Gerencial" }),
      })
    );
  ...
  test("should return 409 when creating cost center with duplicate name", async () => {
    const { headers, organizationId, user } =
      await createTestUserWithOrganization({
        emailVerified: true,
      });
    ...
    await CostCenterService.create({
      organizationId,
      userId: user.id,
      name: "Centro de Custo Duplicado",
    });
    ...
    const response = await app.handle(
      new Request(`${BASE_URL}/v1/cost-centers`, {
        ...
        body: JSON.stringify({ name: "Centro de Custo Duplicado" }),
      })
    );
  ...
  test("should return 409 when creating cost center with duplicate name (case-insensitive)", async () => {
    const { headers, organizationId, user } =
      await createTestUserWithOrganization({
        emailVerified: true,
      });
    ...
    const response = await app.handle(
      new Request(`${BASE_URL}/v1/cost-centers`, {
        ...
        body: JSON.stringify({ name: "centro de custo teste" }),
      })
    );
