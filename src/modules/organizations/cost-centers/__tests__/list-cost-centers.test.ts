...
describe("GET /v1/cost-centers", () => {
  ...
  test("should return empty list when no cost centers exist", async () => {
  ...
  test("should return cost centers for organization", async () => {
  ...
  test("should not return cost centers from other organizations", async () => {
    ...
    expect(body.data[0].name).toBe("Centro de Custo Org 1");
  });

  test("should not return deleted cost centers", async () => {
    const { headers, organizationId, user } =
      await createTestUserWithOrganization({
        emailVerified: true,
      });
  ...
  test("should allow viewer to list cost centers", async () => {
    ...
    await CostCenterService.create({
      organizationId,
      userId: user.id,
      name: "Centro de Custo Viewer Test",
    });
