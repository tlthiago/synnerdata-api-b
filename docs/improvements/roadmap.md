...
| **CP-2** | ✅ **2026-04-24** — Estrutura `src/lib/emails/` criada via `git mv` (history preservada): `mailer.ts` (transporter + sendEmail + sendBestEffort), `senders/{auth,payments,admin,contact}.tsx` (19 senders splittados por domínio), e `{components,templates,render,constants,__tests__}/` movidos de `src/emails/`. Consumers atualizados: 6 services + `lib/auth.ts` + `lib/auth/hooks.ts` + `lib/email-dispatcher.ts` + 2 test files (`mock.module` path + spyOn). `src/lib/email.tsx` (520 linhas) e `src/emails/` deletados. 1425 tests pass em escopo amplo | #8, #9 | refactor | XL | — |
...
| ~~**CP-46**~~ | ~~Migração ultracite 6 → 7 (Biome → Oxc)~~ → **Reclassificado para MP-28 em 2026-04-24** — após executar CP-48 e CP-47 que tinham breaking changes reais, ficou claro que CP-46 é "upgrade by inertia": zero CVE, zero deprecation, zero feature Oxc necessária. Biome 2.x funciona bem hoje. Não vale break risk de re-formatar 600+ arquivos sem ganho concreto. Mesmo critério de CP-44 → MP-27 | — | — | — | — |
| **CP-47** | ✅ **2026-04-24** — better-auth `~1.4.22` → `~1.6.9`. Mudanças aplicadas: (a) `apiKey` movido para `@better-auth/api-key` standalone (1.5); (b) `better-auth-localization` 2.3→3.0 (peer dep); (c) migration 0039: `two_factors.verified` boolean default true; (d) migration 0040: `apikeys.user_id` → `reference_id` + novo `config_id` default 'default'; (e) test `should reject duplicate email` adaptado para nova enumeration protection (200 silent response). `session.freshAge` usa default — sem impacto. 2453+ tests passing em escopo amplo. _Nota: drizzle-kit generate tem bug preexistente em snapshots 35-38; migrations 39-40 criadas manualmente_ | Descoberto em CP-40 | refactor | L | — |
| **CP-48** | ✅ **2026-04-24** — Zod bumped `~4.1.13` → `~4.3.6`. Fix do `.partial() cannot be used on object schemas containing refinements` afetou **apenas 1 arquivo** (`medical-certificates.model.ts`) — outros arquivos com `.partial()` + `.refine()` tinham refines field-level (não object-level) e não disparavam. Pattern: extrair `medicalCertificateFieldsSchema` base, derivar create com `.refine()` e update com `.partial().extend().refine()` a partir da base. Tests validados em escopo grande (952 payments + 201 occurrences + 245 orgs + 101 warnings/labor + 50 medical + 160 auth/admin/public/audit). Remove pin de contenção do CP-40 | Descoberto em CP-40 | refactor | M | — |
...
**Total bucket 🟡: 50 ações registradas · 5 ativas · 40 concluídas (CP-1, CP-2, CP-3, CP-4, CP-5, CP-6, CP-7, CP-8, CP-9, CP-10, CP-11, CP-12, CP-13, CP-20, CP-21, CP-22, CP-23, CP-24, CP-25, CP-26, CP-27, CP-28, CP-29, CP-30, CP-31, CP-32, CP-33, CP-34, CP-35, CP-36, CP-37, CP-38, CP-39, CP-40, CP-42, CP-43, CP-45, CP-47, CP-48, CP-49) · 4 reclassificadas para MP (CP-18 → MP-24, CP-19 → MP-25 em 2026-04-23; CP-44 → MP-27, CP-46 → MP-28 em 2026-04-24) · 1 contenção temporária (CP-50).**
...
| **Onda 5 — Refactors grandes** | ✅ **Concluída em 2026-04-24** | CP-1/3/4/5/6/26/28/32/33/38/2 entregues; CP-44 reclassificado | Onda 5 fechada com CP-2 entregue em 11/11 (100%) |
...
| **Onda 7 — Tooling migrations** ⭐ criada 2026-04-23 | ✅ Encerrada em 2026-04-24 | CP-48 ✅, CP-47 ✅, ~~CP-46~~ → MP-28, CP-50 (contenção) | Migrations com breaking changes reais entregues. CP-46 reclassificado por ser upgrade-by-inertia. CP-50 (TS 6) segue contenção até virar necessidade real |
...
| ✅ | ~~**Onda 7 seq**~~ | Onda 7 | — | — | Encerrada 2026-04-24. CP-48 ✅ + CP-47 ✅; CP-46 → MP-28; CP-50 contenção |
| ✅ | ~~**CP-2** Emails consolidation~~ | Onda 5 | XL | — | Entregue 2026-04-24. Onda 5 fechada em 11/11 |
...
| **MP-28** | Migração ultracite 6 → 7 (Biome → Oxc) — Ultracite 7 troca engine de Biome para Oxc (`oxlint` + `oxfmt`). Implementação-guia: remover `@biomejs/biome` das devDeps, validar `biome.json`/`biome.jsonc` → config equivalente em Oxc, rodar `ultracite check` + `ultracite fix` em todo codebase, validar pre-commit via `lint-staged`. Workspace dedicado + janela de teste recomendados | Ex-CP-46, reclassificado 2026-04-24; Descoberto em CP-40 | Biome 2.x deprecated/EOL OU CVE em Biome OU performance de lint/format virou problema real OU feature específica do Oxc necessária |

**Total bucket 🟢: 28 ações monitoradas (+4 reclassificadas de CP-18/CP-19/CP-44/CP-46 + MP-23/MP-26 formalizados). Nenhuma investida agora — aguardar sinal.**
...
| 🟡 Curto prazo | 50 registradas (40 done · 5 ativas · 4 reclassificadas · 1 contenção) | 4 planos XL/L + ~25 S/M | 30-90 dias | 🔄 Em execução — Ondas 1/2/3/5/6/7 concluídas. Resta apenas trabalho paralelo: CP-41, CP-17, Cloudflare seq, CP-50 (contenção) |
