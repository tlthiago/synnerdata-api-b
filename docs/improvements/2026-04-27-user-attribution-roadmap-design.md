...
- **PRD #4 — Cost-Centers Pilot**: 🟡 em revisão via [PR #312](https://github.com/tlthiago/synnerdata-api-b/pull/312). Implementação alinhada com o design (joins via `auditUserAliases()`, response Zod com `entityReferenceSchema`, 5 testes atualizados + teste novo do path "creator anonimizado" cruzando PRD #2+#3+#4, CLAUDE.md do módulo como referência canônica). CI verde. Pendente 1 ajuste antes do merge: soft-delete deve popular `updatedBy: userId` (Semantic A estrita — decisão tomada em 2026-04-29).
- **Sweep Semantic A em soft-delete**: 🟡 em revisão via [PR #313](https://github.com/tlthiago/synnerdata-api-b/pull/313). Fecha inconsistência herdada de PRD #3 — 6 módulos legados (branches, sectors, job_positions, job_classifications, ppe_items, absences) ganham `updatedBy: userId` no `.set()` do soft-delete; CLAUDE.md raiz formaliza explicitamente que "soft-delete também é UPDATE para essa regra"; 1 teste novo em absences valida o invariante. Cost_centers fica fora deste sweep (será coberto pelo ajuste do PR #312).
- **PRD #5+ — Phase 3 Rollout**: pending merge de #312 + #313. 22 módulos remanescentes replicarão o pattern canônico (cost-centers como referência). Tooling sugerido pelo design: subagent-driven-development.
