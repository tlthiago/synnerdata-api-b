...
| **3. Execução** | 🔄 Em execução | 2026-04-23 | **Bucket 🔴 concluído (10/10)**. Bucket 🟡 **Ondas 1/2/3 quase completas + Onda 5 em andamento** (CP-1 XL + CP-4 L + CP-33 S + CP-26 S + CP-28 S + CP-32 M + CP-5 L + CP-6 M entregues + PR #7 follow-up de CP-6 `ErrorReporter` wrapper fechando débito de testabilidade). Total concluídas no 🟡: **31 CPs** (CP-1, CP-4, CP-5, CP-6, CP-7, CP-8, CP-9, CP-13, CP-20, CP-21, CP-22, CP-23, CP-24, CP-25, CP-26, CP-27, CP-28, CP-29, CP-30, CP-31, CP-32, CP-33, CP-34, CP-35, CP-36, CP-37, CP-39, CP-40, CP-42, CP-43, CP-45). Resta **CP-41** (Onda 3 PR-D). CP-6 reframado: HMAC não é oferecido pelo Pagar.me v5 (research via context7 + SDK), então o escopo virou hardening do Basic Auth existente (Zod validation + auth/skip logs + Sentry capture via `ErrorReporter` wrapper + unhandled event log). Rate limit skip diferido como **MP-22** (volume atual não justifica). Remaining next-up: CP-3 (L routes v1), CP-2 (XL emails). Débito #96 100% endereçado. |
...
1. **Onda 5 — Refactors grandes** (em execução) — entregues: **CP-1 (XL)** `src/plugins/` consolidado, **CP-4 (L)** auth split, **CP-33 (S)** `buildAuditEntry`, **CP-26 (S) + CP-28 (S) + CP-32 (M)** cleanup pós-plugins, **CP-5 (L)** `lib/errors/` cleanup + factory `errorSchema`, **CP-6 (M)** webhook hardening (Basic Auth audit + Zod validation + observability) + **PR #7 follow-up** (`ErrorReporter` wrapper em `src/lib/error-reporter.ts` fechou débito de testabilidade; 3 callsites migrados: webhook + 2× error-plugin). Próximos candidatos: **CP-3 (L)** `src/routes/v1/` + versionamento (destrava CP-18, worktree + plano formal), **CP-2 (XL)** consolidar emails (último).
...
| **CP-4** | ✅ **2026-04-22** — `lib/auth.ts` 856→339 linhas split em `lib/auth/admin-helpers.ts` (getAdminEmails, handleWelcomeEmail), `lib/auth/audit-helpers.ts` (10 auditXxx), `lib/auth/validators.ts` (validateUniqueRole), `lib/auth/hooks.ts` (11 callbacks extraídos — sendResetPassword, onPasswordReset, validação de delete user, databaseHooks, organization lifecycle). `plugins/auth/auth-plugin.ts` 396→79 linhas split em `plugins/auth/options.ts` (AuthOptions/parseOptions), `plugins/auth/validators.ts` (error classes + role/permission/subscription/feature validators), `plugins/auth/openapi-enhance.ts` (OpenAPI helper, consumido direto por `src/index.ts`). Hooks que chamam `auth.api.*` ficam inline em `auth.ts` (ex: `beforeDelete`) para evitar circular; extraímos só a validação. **Destrava CP-33** (consolidação auditXxx em `buildAuditEntry`). | #38, #39, #49, #51 | plan | L | CP-1 |
...
| ID | Ação | Débitos cobertos | Tipo | Esforço | Depende de |
|---|---|---|---|---|---|
| **CP-6** | ✅ **2026-04-22** (+ follow-up 2026-04-23) — **Escopo reframado após research das docs do Pagar.me v5**: HMAC signature não é oferecido pelo gateway (confirmado via context7 + WebSearch + SDK oficial Node.js — zero referências a verify/signature); IP allowlist de origem não é publicada (apenas IPs para chamar a API deles). Sem opções provider-offered, o CP virou hardening da implementação Basic Auth existente: (1) validação Zod declarativa do body via `z.looseObject` (422 em malformação, passthrough para campos extras de Pagar.me); (2) `_rawBody` órfão removido (fecha #57); (3) log `webhook:auth_failure` com reason tag em cada um dos 4 paths de falha do Basic Auth (missing_or_wrong_scheme/invalid_base64/missing_separator/invalid_credentials) + clientIp via `extractClientIp`; (4) log `webhook:skipped:missing-metadata` em `handleChargePaid`/`handleChargeFailed` quando `metadata.organization_id` ausente; (5) `ErrorReporter.capture` com tags `webhook_event_type`+`pagarme_event_id` no catch do processor (via wrapper — ver follow-up abaixo); (6) log `webhook:unhandled-event-type` no switch default. **Follow-up (PR #264)**: `src/lib/error-reporter.ts` novo expõe `ErrorReporter.capture` wrapping `captureException` — destrava testabilidade via property access spyable (`spyOn(ErrorReporter, "capture")`). 3 callsites de prod migrados (webhook + 2× `error-plugin.ts`); `src/lib/sentry.ts` perdeu re-export e virou puramente side-effect. 2 novos tests cobrem Sentry escalation. **Rate limit skip em `/webhooks`** foi auditado mas descartado como intervenção imediata — volume MVP atual (<10 webhooks/dia vs 100 req/min global) torna risco de 429 essencialmente nulo; diferido como **MP-22** (monitored). Testes: 47 baseline → 65 total (7 endpoint + 7 observability + 2 unhandled event + 2 Sentry escalation). | #56 (researched, reframed), #57 (closed) | new→refactor | M | — |
